...
async def show_my_number(message: types.Message):
    ...
    if cached:
        ...
        if is_subscriber:
            prompt = f"Число судьбы {destiny}. Дай развёрнутую характеристику (6-8 предложений): сильные стороны, слабости, ключевой жизненный вызов, совет по самореализации. Будь прямолинеен, но с теплотой. Используй HTML-теги <b>, <i> без <p>."
            response = await get_yandex_gpt_response(prompt, user_id, function_name="number", gender=gender)
            reply_markup = None
        else:
            prompt = f"Число судьбы {destiny}. Дай характеристику (5-6 предложений): укажи 2 сильные стороны, 1 слабость, 1 главную задачу в жизни. В конце добавь фразу: «Хотите узнать, как это число влияет на ваши отношения, карьеру и деньги? Полный разбор – по подписке». Используй HTML-теги <b>, <i> без <p>."
            response = await get_yandex_gpt_response(prompt, user_id, function_name="number", gender=gender)
    ...
    await message.answer(f"🔢 Ваше число судьбы: {destiny}\n\n{response}",
                         reply_markup=reply_markup or quick_topics_menu, parse_mode="HTML")
...
async def process_compatibility(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if is_subscriber:
            prompt = f"Число судьбы пользователя {my_destiny} (знак {my_zodiac}), число партнёра {partner_destiny} (знак {partner_zodiac}). Опиши совместимость развёрнуто (10-12 предложений) по 5 сферам: любовь, дружба, деньги, секс, интеллект. Дай рекомендации, как улучшить отношения. Будь честен и практичен. Используй HTML-теги <b>, <i> без <p>."
            response = await get_yandex_gpt_response(prompt, user_id, function_name="compatibility", gender=gender)
            reply_markup = menu_button
        else:
            prompt = f"Число судьбы пользователя {my_destiny} (знак {my_zodiac}), число партнёра {partner_destiny} (знак {partner_zodiac}). Дай краткое описание совместимости (4-5 предложений). Напиши, что их связывает, что будет сложно, и дай один совет. В конце добавь фразу: «Полный разбор по 5 сферам с рекомендациями – по подписке». Используй HTML-теги <b>, <i> без <p>."
            response = await get_yandex_gpt_response(prompt, user_id, function_name="compatibility", gender=gender)
            reply_markup = get_subscription_button()
        await status_msg.delete()
...
async def daily_card(message: types.Message):
    ...
    if is_subscriber:
        prompt = f"Сегодняшняя карта дня для человека с числом судьбы {destiny}. Дай развёрнутый прогноз (6-8 предложений): общий настрой, практическое действие, психологическая практика, вопрос для рефлексии. Используй HTML-теги <b>, <i> без <p>."
        response = await get_yandex_gpt_response(prompt, user_id, function_name="daily_card", gender=gender)
        reply_markup = menu_button
    else:
        prompt = f"Сегодняшняя карта дня для человека с числом судьбы {destiny}. Дай краткий прогноз (5-6 предложений): что важно сегодня, один практический совет, вопрос, чтобы задуматься. В конце добавь фразу: «Полная карта дня с практиками и погодой – по подписке». Используй HTML-теги <b>, <i> без <p>."
        response = await get_yandex_gpt_response(prompt, user_id, function_name="daily_card", gender=gender)
        reply_markup = get_subscription_button()
    await status_msg.delete()
    last_answer[user_id] = response
    await message.answer(f"🎁 *Карта дня*\n\n{response}{weather_str}", parse_mode="HTML", reply_markup=reply_markup)
...
async def process_consultation(message: types.Message, state: FSMContext):
    ...
    if is_subscriber:
        prompt = f"Человек с числом судьбы {destiny} по имени {name} спрашивает: {question}. Ответь развёрнуто, как психолог, нумеролог, астролог и сексолог (если уместно), с практическими советами. Используй стиль Аркадия Викторовича и HTML-теги <b>, <i> без <p>."
        ...
        await message.answer(response, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    prompt = f"Человек с числом судьбы {destiny} спрашивает: {question}. Дай очень короткий ответ (1-2 предложения), который даст чёткое понимание, но не раскрывай всех деталей. В конце добавь фразу: «Полный разбор и советы – по подписке». Используй HTML-теги <b>, <i> без <p>."
...
async def quick_topic(callback: types.CallbackQuery):
    ...
    is_subscriber = get_user_subscription_status(user_id)
    if is_subscriber:
        prompt = f"Человек с числом судьбы {destiny} спрашивает про {topic}. Дай развёрнутый ответ (5-7 предложений) с практическими советами. Используй HTML-теги <b>, <i> без <p>."
        response = await get_yandex_gpt_response(prompt, user_id, function_name="quick_topic", gender=gender)
        reply_markup = menu_button
    else:
        prompt = f"Человек с числом судьбы {destiny} спрашивает про {topic}. Дай краткий ответ (3-4 предложения) с основной сутью. В конце добавь фразу: «Углублённый разбор и стратегии – по подписке». Используй HTML-теги <b>, <i> без <p>."
        response = await get_yandex_gpt_response(prompt, user_id, function_name="quick_topic", gender=gender)
        reply_markup = get_subscription_button()
    status_msg = await callback.message.answer("🔮 Аркадий Викторович размышляет...")
    await status_msg.delete()
    await callback.message.answer(f"📌 *{topic.capitalize()}*\n\n{response}", parse_mode="HTML", reply_markup=reply_markup or menu_button)
    await callback.answer()
...
async def mynumber_command(message: types.Message):
    ...
    if not row or not row[0]:
        ...
        return
    destiny = row[0]
    await message.answer(f"Ваше число судьбы: *{destiny}*. Хотите подробнее? Нажмите «Моё число» в меню.", parse_mode="HTML")