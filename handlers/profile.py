...
from database import get_connection, get_user, update_user, get_subscription_status  # get_achievements удалён отсюда
from yandex_gpt import get_yandex_gpt_response
from utils import (  # сюда добавлен get_achievements
...
def register_profile_handlers(dp: Dispatcher, bot: Bot, admin_ids: list):
    ...
    async def show_achievements(callback: types.CallbackQuery):
        if callback.message.chat.type in (ChatType.GROUP, ChatType.SUPERGROUP):
            ...
            return
        user_id = callback.from_user.id
        achievements = get_achievements(user_id)  # теперь это из utils
        if not achievements:
    ...
    # ---------- ДОБАВЛЕННЫЕ КОМАНДЫ ДЛЯ ПОДПИСКИ НА РАССЫЛКУ ----------
