...
# ===== ОБРАБОТЧИК ОТМЕНЫ ДЛЯ ПОЛЬЗОВАТЕЛЬСКИХ СОСТОЯНИЙ =====
@router.callback_query(F.data == "cancel_action")
async def cancel_action_callback(callback: types.CallbackQuery, state: FSMContext):
    """Универсальная отмена для пользовательских состояний (возврат в главное меню)."""
    await state.clear()
    await callback.message.delete()  # удаляем сообщение с запросом
    await callback.message.answer("❌ Действие отменено. Возврат в главное меню.", reply_markup=main_menu)
    await callback.answer()